...
function calcASPD()
{
	...
	// Stat ASPD --------------------------------------------------
	statASPD = Math.sqrt( ( n_A_AGI * n_A_AGI / 2  ) + ( n_A_DEX * n_A_DEX /5 ) ) / 4;
	statASPDBow = Math.sqrt( ( n_A_AGI * n_A_AGI / 2  ) + ( n_A_DEX * n_A_DEX /7 ) ) / 4;
	...
	if (n_A_JobSearch2() === cls_BAR && SkillSearch( skill_BA_MUSIC_LESSONS))
	{
		aspdMultiplier += SkillSearch( skill_BA_MUSIC_LESSONS);
	}
